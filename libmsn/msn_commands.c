...
int HandleRing(MSN_Conn *conn, char **args, int numOfArgs)
{
    ...
    */
    ...
    lID = gdk_input_add(newConn->fd, GDK_INPUT_READ, msn_callback_handler, 
                        newConn);
    newConn->listenerID = lID;
    msn_connections = g_list_append(msn_connections, newConn);
    free(newHandle);
    return 0;
}
...
int SendSBYE(MSN_Conn *conn)
{
    ...
    close(conn->fd);
    msn_connections = g_list_remove(msn_connections, conn);
    free(conn);
    return 0;
}
...
int HandleOUT(MSN_Conn *conn, char **args, int numOfArgs)
{
    ...
    if (conn == &mainMSNConnection) {
        ...
        while (node) {
           ...
           gdk_input_remove(connNode->listenerID);
           close(connNode->fd);
           free(connNode);
           node = node->next;
        }
        g_list_free(msn_connections);
...
int Logout(MSN_Conn *conn)
{
    ...
    for (node = msn_connections; node; node=node->next) {
        connNode = node->data;
        SendBYE(connNode);
    }
    return 0;
...
MSN_Conn *FindMSNConnectionByHandle(char *handle)
{
    ...
    for (node = msn_connections; node; node=node->next) {
        connNode = node->data;
        node2 = g_list_find_custom(connNode->chatUsers.users, handle,
                                   CompareUserName);
        if (node2 != NULL)
            return connNode;
    }
...
int KillConnection(MSN_Conn *conn)
{
    ...
    if (conn == &mainMSNConnection) {
        MSN_ErrorOut("You have lost your conenction to the server",
                      "Connection lost.");
...
int main(void) 
{
    ...
    gdk_input_add(msn_connection.fd, GDK_INPUT_READ, msn_callback_handler,
                  &msn_connection);
